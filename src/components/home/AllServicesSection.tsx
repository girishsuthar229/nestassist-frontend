...
import { fetchAllServices, formatPrice } from "@/api/homeServices";
...
const AllServicesSection = ({ searchQuery }: IProps) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        setLoading(true);
        const data = await fetchAllServices(20);
  ...
  const title = searchQuery?.trim()
    ? `${HOME_TEXT.searchResultsTitle} "${searchQuery.trim()}"`
    : HOME_TEXT.allServicesTitle;

  return (
    <section className="pt-12 sm:pt-16" id="all-services">
      <Container>
        <SectionHeading
          title={title}
          action={
